...
def render_dashboard():
    ...
    with left:
        ...
        labels = ["No", "Yes"]
        # Per-cell text color: white on the darker (high-value) cells, dark
        # navy on the lighter ones — a single global color (the old code
        # picked white for every cell) made the low-value cells unreadable
        # against the pale end of the scale.
        cm_max = cm.max() if cm.max() > 0 else 1
        text_colors = [
            ["#FFFFFF" if v >= cm_max * 0.45 else COLORS["text"] for v in row]
            for row in cm
        ]
        fig_cm = go.Figure(
            data=go.Heatmap(
                ...
                text=cm,
                texttemplate="%{text}",
                textfont=dict(size=18, color=text_colors),
                showscale=False,
            )
        )
