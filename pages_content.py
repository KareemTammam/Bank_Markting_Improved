...
def render_predict():
    ...
    if submitted:
        ...
        for col in ["default", "housing", "loan"]:
            display_row[col] = display_row[col].map({1: "yes", 0: "no"})
        st.dataframe(display_row, width='stretch', hide_index=True)

        best_model_name = load_meta().get("best_model_name", type(model).__name__)
        st.caption(
            f"Built with Streamlit • {best_model_name} pre-trained on the Bank Marketing "
            "dataset • model & preprocessor loaded from model/trained_model.pkl"
        )
