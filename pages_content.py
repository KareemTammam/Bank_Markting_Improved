...
from style import COLORS, MODEL_PALETTE, METRIC_COLORS
from utils import (
    build_raw_input_row,
...
def render_home():
    ...
    with left:
        st.markdown('<div class="badge badge-blue">Machine Learning · Binary Classification</div>', unsafe_allow_html=True)
        st.markdown('<div class="hero-title">DepositIQ</div>', unsafe_allow_html=True)
        st.markdown(
...
def render_dataset():
    ...
    st.markdown(
        "<p class='muted'>Direct marketing (phone call) campaign records from a Portuguese "
        "banking institution. The classification target is whether the client subscribed to "
        "a term deposit.</p>",
        unsafe_allow_html=True,
    )
...
def render_dashboard():
    ...
    st.markdown(
        "<p class='muted'>8 classification models were trained and tuned via grid search "
        "cross-validation, then evaluated on a held-out test set.</p>",
        unsafe_allow_html=True,
    )
    ...
    with left:
        ...
        fig_cm = go.Figure(
            data=go.Heatmap(
                ...
                x=labels,
                y=labels,
                colorscale=[[0, "#EFF4FF"], [1, COLORS["primary"]]],
                text=cm,
                texttemplate="%{text}",
                textfont=dict(size=18, color="white" if cm.max() > 0 else "black"),
                showscale=False,
            )
        )
...
def render_predict():
    ...
    st.markdown(
        "<p class='muted'>Fill in the customer's profile and campaign details below. "
        "The exact preprocessing pipeline used during training is applied automatically.</p>",
        unsafe_allow_html=True,
    )
    st.write("")
